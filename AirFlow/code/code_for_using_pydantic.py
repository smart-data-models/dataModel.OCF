...
class AirFlow(BaseModel):
    ...
    automode: Optional[Automode] = Field(
        None,
    ...
    speed: Optional[float] = Field(None, description='The current speed level')
    step: Optional[float] = Field(
        None,
        description='Step value across the defined range when the range is an integer.  This is the increment for valid values across the range; so if range is 0..10 and step is 2 then valid values are 0,2,4,6,8,10',
    )
    supporteddirections: Optional[List[str]] = Field(
        None,
        description='The array of possible direction settings for this instance of the Resource Type',
        min_length=1,
    )
    type: Optional[Type6] = Field(
        None, description='NGSI entity type. It has to be AirFlow'
    )
